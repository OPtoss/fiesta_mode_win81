...
namespace FiestaMode.Pages
{
    ...
    {
        public ColorsModel ViewModel { get; set; }
        ...
        public ColorPickerPage()
        {
            ...
            Debug.WriteLine("Color Picker Page Constructor");

            Loaded += new RoutedEventHandler(ColorPickerPage_Loaded);
        }

        private void ColorPickerPage_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine("Loaded Colors");
            List<ColorItem> item = new List<ColorItem>();
            for (int i = 0; i < 66; i++)
            {
                item.Add(new ColorItem() { Text = colorNames[i], Color = uintColors[i].ToColor() });
            };
        ...
        private void lstColor_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.AddedItems.Count > 0)
            {
                ...
                _selectedColorItem = ((ColorItem)e.AddedItems[0]);
